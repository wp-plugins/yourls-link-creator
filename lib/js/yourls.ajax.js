	...
	$('div#yourls-post-display').on('click', 'input.yourls-api', function (event) {
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			try {
				...
				$('div.yourls-message').delay(3000).slideUp('slow');
				}
			...
			if(obj.success === true) {
				...
				$('div#wpbody h2:first').after('<div id="message" class="error below-h2 yourls-message"><p>' + obj.message + '</p></div>');
				$('div.yourls-message').delay(3000).slideUp('slow');
			}
			else {
			...
			if( obj.yourlsbox !== null ) {
				$('div#yourls-post-display').find('p.yourls-create-block').replaceWith( obj.yourlsbox );
	...
	$('div#yourls-post-display').on('click', 'input.yourls-delete', function (event) {
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if( obj.yourlsbox !== null ) {
				...
				$('div#yourls-post-display').find('p.yourls-exist-block').replaceWith(obj.yourlsbox);
